...
class App extends Component {
  ...
  thumbnailsListItem = eachThumbanil => {
    const {id, thumbnailUrl} = eachThumbanil
  ...
  tabsListItem = eachTab => {
    ...
    const tabClassName =
      activeTabId === tabId ? 'tab-button active' : 'tab-button'
  ...
  renderMatchList = filteredThumbnailsList => {
    const {randomImg} = this.state
    ...
    return (
      <div className="game-items-container">
        <img src={imageUrl} alt="match" className="demo-image" />
        <ul className="tabs-list">
          {tabsList.map(eachTab => this.tabsListItem(eachTab))}
        </ul>
        <ul className="thumbnails-list">
          {filteredThumbnailsList.map(eachThumbanil =>
            this.thumbnailsListItem(eachThumbanil),
          )}
        </ul>
      </div>
    )
  ...
  getFilteredThumbnailsList = imagesLists => {
    const {activeTabId} = this.state
    const filteredThumbnailsList = imagesLists.filter(
      eachThumbanil => eachThumbanil.category === activeTabId,
    )
    return filteredThumbnailsList
  }

  render() {
    const {isGameInProgress, seconds, score} = this.state
    const filteredThumbnailsList = this.getFilteredThumbnailsList(imagesList)
      ...
      <div className="app-container">
        ...
        <div className="game-container">
          <div className="responsive-container">
            {isGameInProgress
              ? this.renderMatchList(filteredThumbnailsList)
